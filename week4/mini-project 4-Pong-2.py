...
wall = [[PAD_WIDTH+BALL_RADIUS, -1.1], 
        [BALL_RADIUS, -1], 
        [WIDTH - PAD_WIDTH - BALL_RADIUS, -1.1], 
...
def draw(canvas):
    ...
    if (ball_pos[1] < wall[TOP_WALL][0]):   # touch top
        ball_vel[1] = wall[TOP_WALL][1]*ball_vel[1]
    elif ball_pos[1] > wall[BOTTOM_WALL][0]:  # touch bottom
            ball_vel[1] = wall[BOTTOM_WALL][1] * ball_vel[1]
            
    if ball_pos[0] < wall[LEFT_WALL][0]: # touch left
        if ball_pos[1] >= paddle[LEFT_PADDLE][0][1] and ball_pos[1] <= paddle[LEFT_PADDLE][0][1]+PAD_HEIGHT: # strike paddle
            ball_vel[0] = ball_vel[0]*wall[LEFT_WALL][1]
        else:
            spawn_ball(RIGHT)
            score2 += 1
    elif ball_pos[0] > wall[RIGHT_WALL][0]: # touch right
            if ball_pos[1] >= paddle[RIGHT_PADDLE][0][1] and ball_pos[1] <= paddle[RIGHT_PADDLE][0][1]+PAD_HEIGHT: # strike paddle
                ball_vel[0] = ball_vel[0]*wall[RIGHT_WALL][1]
            else:
                spawn_ball(LEFT)
                score1 += 1
